...
const PREVIEW_IDS = ["athens", "porto", "split", "valencia", "prague", "rome", "paris", "tokyo"];
...
export const AdditionalDestinations = () => {
  ...
  const visible = PREVIEW_IDS
    .filter((id) => !FEATURED_IDS.includes(id))
    .map((id) => DESTINATIONS.find((d) => d.id === id))
    .filter(Boolean);

  return (
    <section id="additional-destinations" className="border-b border-[#2A2624] bg-[#F4EFE6]">
        ...
        <div className="grid md:grid-cols-12 gap-10 mb-12 items-end">
          <div className="md:col-span-7">
            ...
            <h2 className="font-serif text-4xl md:text-5xl font-black tracking-tighter">
              A few more places <span className="italic font-normal">worth opening.</span>
            </h2>
          </div>
          <p className="md:col-span-5 text-[#2A2624]/80 text-lg leading-relaxed">
            A short preview only. The full destination database powers matching and city pages, without turning the homepage into a giant list.
          </p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {visible.map((dest, i) => {
            ...
            return (
              <article
                key={dest.id}
                data-testid={`additional-destination-${dest.id}`}
                className="bg-[#EBE4D8] border border-[#2A2624] shadow-stamp-sm press-effect flex flex-col reveal"
                style={{ animationDelay: `${i * 45}ms` }}
              >
                <div className="aspect-[4/3] overflow-hidden border-b border-[#2A2624] relative">
                  <UnsplashImage
                    imageKey={`city:${dest.id}:additional-card`}
                ...
                <div className="p-5 flex-1 flex flex-col">
                  ...
                  <p className="font-serif italic text-sm text-[#2A2624]/70 mt-2 leading-relaxed">{dest.blurb}</p>
                  ...
                  <div className="mt-auto pt-4 border-t border-dashed border-[#2A2624]/40 flex items-end justify-between gap-3">
                    <div>
                    ...
                    <button
                      ...
                      data-testid={`additional-open-${dest.id}`}
                      className="press-effect inline-flex items-center gap-2 bg-[#2A2624] text-[#F4EFE6] font-mono text-[10px] uppercase tracking-[0.2em] px-3 py-2.5 shadow-stamp-sm border border-[#2A2624] shrink-0"
                    >
